/**
 * LEADYFY OS - Production Database Migration Script (CommonJS)
...
async function migrate() {
  ...
  try {
    ...
    // 2. Creator
    {
      const rows = sqlite.prepare(`SELECT * FROM "Creator"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 3. ActivityLog
    {
      const rows = sqlite.prepare(`SELECT * FROM "ActivityLog"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 4. Employee
    {
      const rows = sqlite.prepare(`SELECT * FROM "Employee"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 5. Client
    {
      const rows = sqlite.prepare(`SELECT * FROM "Client"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    {
      const rows = sqlite.prepare(`SELECT * FROM "CreatorAvailability"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 7. Expense
    {
      const rows = sqlite.prepare(`SELECT * FROM "Expense"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 8. Task
    {
      const rows = sqlite.prepare(`SELECT * FROM "Task"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 10. Order
    {
      const rows = sqlite.prepare(`SELECT * FROM "Order"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 11. SupportTicket
    {
      const rows = sqlite.prepare(`SELECT * FROM "SupportTicket"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 12. Asset
    {
      const rows = sqlite.prepare(`SELECT * FROM "Asset"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 13. Script
    {
      const rows = sqlite.prepare(`SELECT * FROM "Script"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 14. Payment
    {
      const rows = sqlite.prepare(`SELECT * FROM "Payment"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 15. TicketResponse
    {
      const rows = sqlite.prepare(`SELECT * FROM "TicketResponse"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 16. ScriptComment
    {
      const rows = sqlite.prepare(`SELECT * FROM "ScriptComment"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 17. Shoot
    {
      const rows = sqlite.prepare(`SELECT * FROM "Shoot"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 18. Video
    {
      const rows = sqlite.prepare(`SELECT * FROM "Video"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 19. VideoFeedback
    {
      const rows = sqlite.prepare(`SELECT * FROM "VideoFeedback"`).all();
      let count = 0;
      for (const r of rows) {
    ...
    // 20. CreatorPayout
    {
      const rows = sqlite.prepare(`SELECT * FROM "CreatorPayout"`).all();
      let count = 0;
      for (const r of rows) {
